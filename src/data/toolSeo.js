...
const toolSeo = {
  ...
  'remove-watermark': {
    ...
    faqs: [
      { q: 'Does this really delete the watermark from a PDF?', a: 'It reprints each occurrence in a solid colour matched to the page, right on top of the original letters, so it is visually gone. It deliberately doesn\'t fill a big box around the text — that could blank out real content the watermark happens to cross — so only the watermark\'s own ink gets covered. The original text stays underneath in the file rather than being stripped out — for pages you\'ll print or view normally that looks identical to a true removal, but the text is technically still there if someone dug through the file\'s raw content.' },
