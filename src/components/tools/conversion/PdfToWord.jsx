...
const PdfToWord = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setServer(res.data?.pdf_to_word ? 'ready' : 'unavailable');
      } catch (_) {
  ...
  const build = async () => {
    ...
    try {
      ...
      const res = await api.post('/convert/pdf-to-word', fd, { responseType: 'blob', timeout: 300000 });
      let blob = res.data;
      if (blob?.type && blob.type.indexOf('word') === -1 && blob.type.indexOf('octet-stream') === -1) {
  ...
  const sidebar = (
    <>
      ...
      <section className="space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        />
        <p className="text-[11px] text-gray-400 dark:text-gray-500">Leave empty to convert the whole document.</p>
      </section>

      <section className="pt-4 border-t border-gray-200 dark:border-gray-700">
        <p className="text-[11px] text-gray-500 dark:text-gray-400">
          The PDF is rebuilt into an editable Word document — paragraphs, tables, images and fonts are
          reconstructed from the page layout. Scanned/image-only PDFs come through as pictures (no OCR).
  ...
  const resultView = (working || result) ? (
    <ResultScreen
      ...
      workingLabel="Converting to Word…"
      fileName={outName}
      fileSize={result?.size}
      onDownload={() => downloadBlob(result.blob, outName)}
      onBack={backFromResult}
      backLabel="Convert another PDF"
      note="Rebuilt from the page layout — check tables and spacing. The file stays on your device."
    />
  ) : null;
  ...
  return (
    <ToolWorkspace
      ...
      formats={`PDF up to ${SERVER_UPLOAD_MB.convert} MB — turned into an editable Word .docx`}
    ...
    >
      ...
      {server === 'unavailable' && (
        <p className="mt-4 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded-lg px-3 py-2">
          The PDF&nbsp;→&nbsp;Word converter isn&apos;t responding right now. Please try again in a little while.
        </p>
      )}
    </ToolWorkspace>
  );
