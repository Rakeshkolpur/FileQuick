...
const PdfToPowerPoint = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setServer(res.data?.pdf_to_powerpoint ? 'ready' : 'unavailable');
      } catch (_) {
  ...
  const build = async () => {
    ...
    try {
      ...
      const res = await api.post('/convert/pdf-to-powerpoint', fd, { responseType: 'blob', timeout: 300000 });
      let blob = res.data;
      if (blob?.type && blob.type.indexOf('presentation') === -1 && blob.type.indexOf('octet-stream') === -1) {
        const text = await blob.text();
        throw new Error(JSON.parse(text).error || text);
      }
      blob = new Blob([blob], { type: PPTX_MIME });
      setResult({ blob, size: blob.size });
    } catch (e) {
      console.error(e);
      let msg;
      if (e.response?.status === 413) msg = `That PDF is too large — the converter takes files up to ${SERVER_UPLOAD_MB.convert} MB.`;
  ...
  const outName = `${stripExt(file?.name || 'document')}.pptx`;
  ...
  const ServerBadge = () => {
    const map = {
      ...
      unavailable: ['bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300', 'Converter unavailable — try again shortly'],
    };
  ...
  const sidebar = (
    <>
      ...
      <section className="space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
        <label className="block text-sm font-semibold text-gray-900 dark:text-white">Pages to convert</label>
        <input
          type="text"
          value={pagesSpec}
        ...
        />
        <p className="text-[11px] text-gray-400 dark:text-gray-500">One slide per page. Leave empty for the whole document.</p>
      </section>

      <section className="pt-4 border-t border-gray-200 dark:border-gray-700">
        <p className="text-[11px] text-gray-500 dark:text-gray-400">
          Each PDF page becomes a full-size slide at its original proportions, so the deck looks
          exactly like the PDF. Add your own text boxes on top in PowerPoint afterwards.
          Processed on your machine and deleted right after.
        </p>
      </section>
  ...
  const resultView = (working || result) ? (
    <ResultScreen
      ...
      title="Your PowerPoint deck is ready"
      workingLabel="Building slides…"
      fileName={outName}
      fileSize={result?.size}
      onDownload={() => downloadBlob(result.blob, outName)}
      onBack={backFromResult}
      backLabel="Convert another PDF"
      note="One slide per page, rendered from the PDF. The file stays on your device."
    />
  ) : null;
  ...
  return (
    <ToolWorkspace
      ...
      formats={`PDF up to ${SERVER_UPLOAD_MB.convert} MB — one slide per page in a .pptx deck`}
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-3">
        <p className="text-sm font-medium text-gray-900 dark:text-white">
          {phase === 'ready' ? `${pageCount} page${pageCount === 1 ? '' : 's'} → ${pageCount} slide${pageCount === 1 ? '' : 's'}` : 'Preview'}
        </p>
      </div>
      ...
      <div className="rounded-xl bg-gray-100 dark:bg-gray-900/40 p-3 sm:p-5 max-h-[70vh] overflow-auto">
        {phase === 'loading' && thumbs.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {thumbs.map((t) => (
              <div key={t.page} className="rounded-lg overflow-hidden bg-white shadow ring-1 ring-black/5" style={{ width: 168 }}>
                <img src={t.dataUrl} alt={`Page ${t.page}`} className="w-full block" />
                <div className="text-[11px] text-center text-gray-500 py-1">Slide {t.page}</div>
              </div>
            ))}
            {pageCount > thumbs.length && phase === 'ready' && (
              <div className="flex items-center justify-center text-xs text-gray-400" style={{ width: 168 }}>
      ...
      {server === 'unavailable' && (
        <p className="mt-4 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded-lg px-3 py-2">
          The PDF&nbsp;→&nbsp;PowerPoint converter isn&apos;t responding right now. Please try again in a little while.
        </p>
      )}
    </ToolWorkspace>
  );
