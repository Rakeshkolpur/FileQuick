...
const ImageResize = () => {
  ...
  // Single-image editing screen: which tab of the right-hand panel is open.
  ...
  // Current edited image (rotation/crop/background applied) as a blob, for
  // handing off to another tool mid-edit. Falls back to the original file
  // untouched when nothing has been changed yet, to avoid a needless re-encode.
  const getCurrentImage = () => new Promise((resolve) => {
    if (!single) { resolve(null); return; }
    const hasEdits = rotation !== 0 || bgRemove || cropApplied;
    if (!hasEdits || !workSource) { resolve(single.file); return; }
    const w = cropRect ? Math.round(cropRect.width) : natW;
    const h = cropRect ? Math.round(cropRect.height) : natH;
    const c = document.createElement('canvas');
    c.width = Math.max(1, w);
    c.height = Math.max(1, h);
    const ctx = c.getContext('2d');
    if (cropRect) ctx.drawImage(workSource, cropRect.x, cropRect.y, cropRect.width, cropRect.height, 0, 0, c.width, c.height);
    else ctx.drawImage(workSource, 0, 0);
    c.toBlob((b) => resolve(b || single.file), 'image/png');
  });
  ...
  // The Back button normally routes through ToolWorkspace, but the single-image
  // editor below renders its own layout instead of <ToolWorkspace> — wire the
  // same "Back returns to the upload screen" behaviour here too.
  const registerBack = useContext(ToolBackContext);
  useEffect(() => {
    const back = (busy || anyResult) ? backFromResult : reset;
    registerBack(single ? back : null);
    return () => registerBack(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [single, busy, anyResult, registerBack]);
  ...
  // Dedicated single-image editing layout: canvas card + a tabbed panel
  // (Download / Tools) on the right. Batch mode, the empty dropzone, and the
  // result/download screen all still go through <ToolWorkspace> below, so
  // this only changes what you see once a single image is loaded.
  if (single && !busy && !anyResult) {
    return (
        ...
        <aside className="lg:sticky lg:top-24 rounded-2xl border border-gray-200/70 dark:border-gray-700/60 bg-white dark:bg-gray-800 flex flex-col overflow-hidden lg:max-h-[calc(100vh-7rem)]">
          ...
          <div className="flex-1 lg:overflow-y-auto p-4 space-y-4">
            {activeTab === 'download' ? (
              <>
                <section className="space-y-3">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Resize</h3>
                  <Segmented
                    options={[
                      { value: 'filesize', label: 'File size' },
                      { value: 'percent', label: 'Percentage' },
                  ...
                  {isSizeMode && (
                    <div className="space-y-3">
                      <div>
                        <span className="block mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">Target size</span>
                        <div className="flex gap-2">
                          <input type="number" min="1" placeholder="e.g. 50" value={targetValue} onChange={(e) => { setTargetValue(e.target.value); markDirty(); }} className={numField} />
                          <select value={targetUnit} onChange={(e) => { setTargetUnit(e.target.value); markDirty(); }} className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm px-2">
                            <option>KB</option>
